...
@torch.no_grad()
def greedy_generate(model, tokenizer, input_ids, past_key_values, max_gen_len, output_file, repetition_penalty=1.2):
    outputs = model(
        input_ids=input_ids,
    ...
    for _ in range(max_gen_len - 1):
        ...
        for idx in range(len(generated_ids)):
            logits[:, generated_ids[idx]] /= repetition_penalty
